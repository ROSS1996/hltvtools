...
def main():
    ...
    url = f"https://www.hltv.org/team/{team_id}/{team_name}#tab-rosterBox"

    try:
        html = get_html(url)
        team_data = parse_team_data(html, team_id)
        save_to_json(team_data, f"{team_name}")
...
def save_to_json(team_data, team_name):
    print(team_data)
    ...
    print(f"Team data saved to {file_name}")
