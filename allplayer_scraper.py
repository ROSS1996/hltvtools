...
def main():
    ...
    filter_data = input(
        "Do you want to filter the data by date range? (y/n) ").lower()
    ...
    for team in teams:
        ...
        for player in players:
            if filter_data in ['y', 'yes']:
                scrape(player['playerId'], player['playerLinkName'],
...
def scrape(player_id, player_name, filter_data=False, start_date=None, end_date=None):
    ...
    try:
        ...
        save_player(player_data, f"{player_name}")
...
def save_player(player_data, player_name):

    all_players["players"].append(player_data)
...
def save_to_json():
    ...
    file_name = f"players.json"
    # Write the player data to the file
    with open(file_name, 'w') as f:
        json.dump(all_players, f, indent=4)

    print(f"Scrape job finished")
